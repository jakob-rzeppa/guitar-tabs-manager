...
export async function fetchAllSheets(options: { force?: boolean } = {}): Promise<void> {
    ...
    if (!options.force && sheetsStore.sheetsList.length > 0) return;
    ...
    await useApiInStore<SheetListItemDto[]>({
        ...
        onSuccess: ({ data }) => {
            if (!data.payload) {
                sheetsStore.error = 'Request content is empty';
                return;
            }

            sheetsStore.sheetsList = data.payload.map((sheet) => ({
                id: sheet.id,
                title: sheet.title,
                artist: sheet.artist,
                tags: sheet.tags,
...
export async function fetchSheet(id: string, options: { force?: boolean } = {}): Promise<void> {
    ...
    if (!options.force && sheetsStore.detailedSheets[id]) return;
    ...
    await useApiInStore<SheetDto>({
        ...
        onSuccess: ({ data }) => {
            if (!data.payload) {
                sheetsStore.error = 'Request payload is empty';
                return;
            }

            sheetsStore.detailedSheets[id] = {
                id: data.payload.id,
...
export async function createSheet(sheet: Omit<Sheet, 'id'>): Promise<void> {
    ...
    await useApiInStore<SheetDto>({
        ...
        onSuccess: ({ data }) => {
            ...
            sheetsStore.detailedSheets[data.payload.id] = {
                id: data.payload.id,
                title: data.payload.title,
                artist: data.payload.artist,
                tags: data.payload.tags,
...
export async function updateSheet(id: string, fieldsToUpdate: Partial<Omit<Sheet, 'id'>>): Promise<void> {
    ...
    const currentSheet = sheetsStore.detailedSheets[id];
    ...
    await useApiInStore<SheetDto>({
        ...
        onSuccess: ({ data }) => {
            ...
            const index = sheetsStore.sheetsList.findIndex((sheet) => sheet.id === Number(id));
            if (index !== -1) {
                sheetsStore.sheetsList[index] = {
                    id: data.payload.id,
                    title: data.payload.title,
                    artist: data.payload.artist,
                    tags: data.payload.tags,
...
export async function deleteSheet(id: string): Promise<void> {
    ...
    await useApiInStore<DeleteDto>({
        ...
        onSuccess: () => {
            delete sheetsStore.detailedSheets[id];

            // Also remove from sheetsList
            const index = sheetsStore.sheetsList.findIndex((sheet) => sheet.id === Number(id));
            if (index !== -1) {
                sheetsStore.sheetsList.splice(index, 1);
            }
        },
    });
}