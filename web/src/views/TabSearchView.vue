...
<template>
  ...
  <main v-else class="bg-base-200 md:w-2/3 max-w-4xl mx-auto border-base-300 border-x-2 min-h-screen">
    <ul class="grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-2 p-8">
      <li v-for="tab in response.data.content" class="card shadow-sm bg-base-100 cursor-pointer hover:shadow-lg transition-shadow" @click="router.push({name: 'tab', params: {id: tab.id}});">
        <div class="card-body">
