...
import type { CreateTabRequest, DeleteDto, TabDto, TabListItemDto } from '@/types/dtos';
...
export const useTabsStore = defineStore('tabs', {
    ...
    actions: {
        ...
        async fetchAllTabs(options: { force?: boolean } = {}): Promise<void> {
            ...
            await useApiInStore<TabListItemDto[]>({
                store: this,
                ...
                onSuccess: ({ data }) => {
                    ...
                    this.tabsList = data.content.map((tab) => ({
                        id: tab.id,
                        title: tab.title,
                        artist: tab.artist,
                        tags: tab.tags,
        ...
        async createTab(tab: Omit<Tab, 'id'>): Promise<void> {
            ...
            await useApiInStore<TabDto>({
                ...
                onSuccess: ({ data }) => {
                    ...
                    this.detailedTabs[data.content.id] = {
                        id: data.content.id,
                        title: data.content.title,
                        artist: data.content.artist,
                        tags: data.content.tags,
